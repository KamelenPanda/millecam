...
import { ARTICLES_FR, getArticleFr } from "@/lib/insights";

export function generateStaticParams() {
  return ARTICLES_FR.map((a) => ({ slug: a.slug }));
}
  ...
  return (
    <Section className="pb-24 pt-16">
      <Reveal className="mx-auto max-w-2xl">
        <Link href="/fr/perspectives" className="text-sm text-ink/60 hover:text-terracotta hover:underline">
        ...
        <p className="mt-4 text-xs text-muted">
          Robin Millecam · {formatDate(article.publishedAt)} · {article.readingTime} de lecture
        </p>
        ...
        <div className="mt-14 border-l-2 border-terracotta bg-[#FBF9F4] py-6 pl-6 pr-5 shadow-[0_14px_34px_rgba(33,29,24,0.09)]">
          ...
          <p className="mt-3 text-sm text-ink/70">
            Envie de savoir comment cela se présente pour votre organisation ?{" "}
            <Link href={article.relatedHref} className="text-terracotta hover:underline">
              {article.relatedLabel}
            </Link>
            , ou{" "}
            <Link href="/fr/contact" className="text-terracotta hover:underline">
              planifiez directement un entretien
            </Link>
            .
          </p>
        </div>

        <Button href="/fr/contact" variant="primary" className="mt-8">
