...
export default function FrameworkGrid({ frameworks }: { frameworks: Framework[] }) {
  ...
  return (
    <div ref={ref} className="grid gap-6 md:grid-cols-2">
      {frameworks.map((f, i) => {
        const className =
          "flex items-start gap-5 border-l-2 border-terracotta bg-[#FBF9F4] py-6 pl-6 pr-4 shadow-[0_14px_34px_rgba(33,29,24,0.09)] transition-all duration-500 ease-out" +
          (f.href ? " hover:bg-white" : "");
        const style = {
          opacity: visible ? 1 : 0,
        ...
        return f.href ? (
          <Link key={f.naam} href={f.href} className={className} style={style}>
            {content}
          </Link>
        ) : (
          <div key={f.naam} className={className} style={style}>
            {content}
          </div>
        );
      })}
