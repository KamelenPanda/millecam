import { ReactNode } from "react";

type ServiceCardProps = {
  naam: string;
  vorm: string;
  voorWie: string;
  watJeKrijgt: string;
  icon?: ReactNode;
  accent?: string;
};

export default function ServiceCard({
  naam,
  vorm,
  voorWie,
  watJeKrijgt,
  icon,
  accent = "border-prussian",
}: ServiceCardProps) {
  return (
    <div className={`h-full border-l-2 ${accent} bg-[#F4F5F2] py-5 pl-5 pr-4 shadow-[0_14px_34px_rgba(20,24,29,0.09)]`}>
      {icon && <div className="mb-3">{icon}</div>}
      <h3 className="font-serif text-xl font-semibold text-ink">{naam}</h3>
      <p className="mt-1 text-sm font-medium text-prussian">{vorm}</p>
      <p className="mt-4 text-sm text-ink/70">
        <span className="font-medium text-ink">Voor wie: </span>
        {voorWie}
      </p>
      <p className="mt-2 text-sm text-ink/70">
        <span className="font-medium text-ink">Wat je krijgt: </span>
        {watJeKrijgt}
      </p>
    </div>
  );
}
